    ...
    <xsl:template match="/">
        ...
        <svg viewBox="0 0 400 -300">
            ...
            <g transform="translate(50, 500)">
                <xsl:for-each select="//persName[.!='Beverages' and .!='Dessert' and .!='Water' and .!='Coffee' and .!='Tea' and .!='Decaf/Tea'] ! lower-case(.) => distinct-values()">   
                    ...
                    <xsl:if test="position() lt 11">
                        ...
                        <g id="{translate(current(), ' ', '_')}">
                            ...
                            <text x="{$xlabel}" y="20" fill="#000000"  font-weight="bold" style="text-anchor: middle" font-size="10" font-family="Georgia"><xsl:value-of select="($text//persName[lower-case(.)=current()])[1]"/></text>
                            <line x1="{$xlabel}" y1="0" x2="{$xlabel}" y2="{$ypos}" stroke-width="40" stroke="#F7D723"/>
                            <text x="{$xlabel}" y="{$ypos - 10}" fill="black" font-weight="bold" font-size="20" font-family="Georgia" style="text-anchor: middle"><xsl:value-of select="count($text//persName[lower-case(.)=current()])"/></text>
                        </g></xsl:if> 
                </xsl:for-each>
                ...
                <!-- general labels -->
                <text x="{10 * $xint div 2}" y="{10 * $ystretch - 10}" font-family="Georgia" fill="#AD1800" style="text-anchor: middle" font-size="35">10 Most Common People Mentioned at Harambee Dinners</text>           
                <text x="{10 * $xint div 2}" y="70" fill="#AD1800" style="text-anchor: middle" font-family="Arial" font-size="20">Name</text>
                <text x="{($ystretch * 10) div 2}" y="230" fill="#AD1800" style="text-anchor: middle" transform="rotate(90,100,100)" font-family="Arial" font-size="18">Number of Mentions</text>
            </g>
        </svg>
    </xsl:template>
